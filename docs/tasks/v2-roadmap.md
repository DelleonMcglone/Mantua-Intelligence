...
| 🗣️ Phase N: Natural Language Command Bar             | 11    | 5    | `█████░░░░░` 45% (+6 🟡 — UI / OpenAI fallback / E2E deferred) |
...
| ID     | Task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Status |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ |
| PN-006 | Build `<CommandBar />` component per design prototype — appears on Swap, Liquidity, Agent pages. **UI deferred** to TD-004 (item 11 added). Server endpoint `POST /api/command/parse` is the data layer underneath.                                                                                                                                                                                                                                                                                                                                                            | 🟡     |
...
| PN-008 | Preview card: after parsing, show the parsed intent as a structured preview BEFORE the confirmation modal. **UI deferred** to TD-004. Parser already returns the structured intent + raw text + confidence + page context in the response so the preview renderer has everything it needs.                                                                                                                                                                                                                                                                                     | 🟡     |
| PN-009 | Clarification loop: if confidence < threshold OR required param missing, LLM asks a follow-up question instead of executing. **Server-side shipped** — `clarification_needed` intent carries a specific `message` for the user plus an optional `suggestedAction` field for the preview card. UI loop (capture user's reply, append to conversation, re-parse) deferred to TD-004.                                                                                                                                                                                             | 🟡     |

### Safety

| ID     | Task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Status |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| PN-010 | Safety rail: confirmation modal (P1-005) is mandatory for every parsed intent that triggers a tx — LLM output never executes directly. **Confirmation seam exists** at `client/src/hooks/use-confirmed-action.tsx` (Phase D). The wiring (parsed intent → `useConfirmedAction({...})` → action endpoint) lives in the CommandBar UI which is deferred to TD-004. The server side never auto-executes — `POST /api/command/parse` returns the intent and stops there; execution always happens through a separate authenticated POST to the matching action endpoint, which the user must trigger. The architectural invariant is in place; the UI plumbing follows. | 🟡     |
