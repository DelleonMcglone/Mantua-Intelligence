...
| B6-009 | Portfolio: open market positions with entry price, current implied odds, unrealised P/L, and market status — ✅ LIVE 2026-08-18 — `/api/markets/positions` marks YES/NO holdings at the live pool price in the profile; entry price/realized P&L noted as pending indexed fills | 🔴 P0    | ⏸      |
...
| B7-003 | Swap handles market outcome tokens and base tokens (USDC / EURC / cirBTC) — ✅ buy flow LIVE 2026-08-18 — Trade button on tradeable matchup cards → quote via market V4Quoter → user-signed PoolSwapTest swap; sell/close direction next (API supports it) | 🔴 P0    | ⏸      |
...
| B10-005 | Agent E2E: natural language → parse → preview → confirm → execute → audit entry — ⏸ parse → preview → confirm → audit proven (B9-004 flow + agent confirm); the execute leg is gated on the periphery deploy with the rest of position execution                                                                                                                                     | 🔴 P0    | ⏸      |
| B10-006 | Hedging E2E: arm strategy → trigger fires → executes under cap → disarms on freeze — ⏸ decision layer proven end-to-end (hold → fire → cap-clamped size → disarm-at-kickoff, `strategies.test.ts` B10-006); execute-under-cap leg gated on periphery                                                                                                                                 | 🟠 P1    | ⏸      |
...
| B10-008 | Testnet posture verified end to end (DM-108). No testnet notice in the UI per the DM-108 close — ✅ marketing surfaces (landing, hero, features) carry no testnet notice; only operational docs (faucet, chain reference) name Arc Testnet factually — DM-108 compliant                                                                                                              | 🔴 P0    | ✅     |
...
| B10-010 | Ship — ⏸ PIPELINE LIVE 2026-08-17: prod schema applied, first sync minted 8 WNBA markets + opened 8 pools at implied odds, zero failures (verified on-chain). Remaining gates: owner sign-off (B10-007) and the outcome-token trading UI                                                                                                                                             | 🔴 P0    | ⏸      |
