...
verified that no Mantua hook lives on Base Mainnet (8453). Phase 5b
pivots Mantua to Base Sepolia (84532) — Stable Protection needs to be
deployed there. Phase 5b-3 ships the parameterized deploy script
...
2. Re-runs `npm run verify:hooks` so Stable Protection's row in
   `docs/security/hook-deployments.md` shows Base Sepolia (84532) ✅.
3. Updates `docs/security/sign-off.md` to mark the bytecode-verified
   column ✅ for Stable Protection on Base Sepolia.
4. Updates `contracts/README.md` hook table (add the Base Sepolia row).
